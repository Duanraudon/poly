...
import (
	...
	"github.com/polynetwork/poly/p2pserver/message/utils"
...
func (this *P2PServer) retryInactivePeer() {
	...
	if len(this.RetryAddrs) > 0 {
		...
		for addr, v := range this.RetryAddrs {
			...
			log.Infof("try connect %v num:%v", addr, v)
			list[addr] = v
		}
		...
		for _, addr := range addrs {
			rand.Seed(time.Now().UnixNano())
			log.Infof("[p2p]Try to reconnect peer, peer addr is %v", addr)
...
func (this *P2PServer) keepOnlineService() {
	t := time.NewTimer(time.Second * common.CONN_MONITOR * 2)
	for {
		select {
		case <-t.C:
	...
	for _, p := range peers {
		if p.GetSyncState() == common.ESTABLISH {
			...
			if t.Before(time.Now().Add(-1 * time.Second *
				time.Duration(periodTime) * common.KEEPALIVE_TIMEOUT * 3)) {
				log.Warnf("[p2p]keep alive timeout!!!lost remote peer %d from %s on %s", p.GetID(), p.SyncLink.GetAddr(), t.String())
				p.CloseSync()
				p.CloseCons()
			}
		}
