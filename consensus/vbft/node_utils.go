...
func (self *Server) getProposerRankLocked(blockNum uint32, peerIdx uint32) int {
	if blockNum == self.currentParticipantConfig.BlockNum {
		...
		log.Warnf("todo: get proposer config for non-current blocknum:%d, current.BlockNum%d,peerIdx:%d", blockNum, self.currentParticipantConfig.BlockNum, peerIdx)
	}
...
func (self *Server) heartbeat() {
	...
	log.Info("broadcast heartbeat to all peer")
	//	send to peer
...
func (self *Server) receiveFromPeer(peerIdx uint32) (uint32, []byte, error) {
	...
	return 0, nil, fmt.Errorf("peerIdx:%v nil consensus payload", peerIdx)
}
