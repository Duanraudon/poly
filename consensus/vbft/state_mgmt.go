...
func (self *StateMgr) onPeerUpdate(peerState *PeerState) error {
	...
	log.Infof("server %d peer update, current build blk %d, state %d, from peer %d, committed blk %d",
...
func (self *StateMgr) onLiveTick(evt *StateEvent) error {
	log.Infof("onLiveTick,blockNum:%v,lastTickChainHeight:%v,state:%v", evt.blockNum, self.lastTickChainHeight, self.getState())
...
func (self *StateMgr) canFastForward(targetBlkNum uint32) bool {
	...
	for blkNum := self.server.GetCurrentBlockNo(); blkNum <= targetBlkNum; blkNum++ {
		...
		if proposer == math.MaxUint32 {
			log.Warnf("server %d check fastforward false, no consensus in %d commit msg for block %d",
		...
		if !foundProposal {
			log.Warnf("server %d check fastforward false, no proposal for block %d",
