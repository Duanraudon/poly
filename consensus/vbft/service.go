...
func NewVbftServer(account *account.Account, txpool, p2p *actor.PID) (*Server, error) {
	...
	if err := server.initialize(); err != nil {
		return nil, fmt.Errorf("vbft server initialize failed: %s", err)
	}
	return server, nil
}
...
func (self *Server) run(peerPubKey keypair.PublicKey) error {
	...
	go func() {
		for {
			...
			if err != nil {
				log.Errorf("receive from peer:%v,err:%v", peerID, err)
				errC <- err
				return
			}
			...
			if err != nil {
				...
				if msg.Type() < PeerHeartbeatMessage {
...
func (self *Server) onConsensusMsg(peerIdx uint32, msg ConsensusMsg, msgHash common.Uint256) {
	...
	case BlockEndorseMessage:
		...
		if msgBlkNum > self.GetCurrentBlockNo() {
			...
			if msgBlkNum <= self.GetCommittedBlockNo() {
				if msgBlkNum+MAX_SYNCING_CHECK_BLK_NUM < self.GetCommittedBlockNo() {
					log.Infof("server %d get endorse msg for block %d, from endorser %d, current committed %d",
	...
	case DeleteSealedBlockMessage:
		pMsg, ok := msg.(*deleteSealedBlockMsg)
		if !ok {
			log.Error("invalid msg with proposal msg type")
			return
		}
		...
		if self.GetCurrentBlockNo() == msgBlkNum+1 {
			self.deleteSealedBlock(msgBlkNum)
		} else {
			log.Warnf("current block num %v is not equal to msg blk num %v", self.GetCurrentBlockNo(), msgBlkNum+1)
		}
	}
}

func (self *Server) deleteSealedBlock(blockNum uint32) {
	c := self.blockPool.candidateBlocks[blockNum]
	if c != nil && c.SealedBlock != nil {
		h := c.SealedBlock.Block.Hash()
		if bytes.Compare(h[:], common.UINT256_EMPTY[:]) != 0 {
			delete(self.blockPool.candidateBlocks, blockNum)
			log.Infof("deleteSealedBlock %d from pool.candidateBlocks", blockNum)
		}
	}
	log.Infof("deleteSealedBlock from chainStore.pendingBlocks, blk: %d", blockNum)
	delete(self.blockPool.chainStore.pendingBlocks, blockNum)
...
func (self *Server) processProposalMsg(msg *blockProposalMsg) {
	...
	if self.LastConfigBlockNum != math.MaxUint32 && blk.Info.LastConfigBlockNum != self.LastConfigBlockNum {
		log.Errorf("BlockProposalMessage check LastConfigBlockNum blocknum:%d,prvLastConfigBlockNum:%d,self LastConfigBlockNum:%d", msg.GetBlockNum(), blk.Info.LastConfigBlockNum, self.LastConfigBlockNum)
		return
	}
	...
	if crossStateRoot != msgCrossStateRoot {
		log.Errorf("BlockProposalMessage check crossStateRoot blocknum:%d,msg crossStateRoot:%s,self crossStateRoot:%s", msg.GetBlockNum(), msgCrossStateRoot.ToHexString(), crossStateRoot.ToHexString())
		return
	}
	...
	if currentBlockTimestamp <= prevBlockTimestamp || currentBlockTimestamp > uint32(time.Now().Add(time.Minute*10).Unix()) {
		log.Errorf("BlockProposalMessage check blocknum:%d,prevBlockTimestamp:%d,currentBlockTimestamp:%d", msg.GetBlockNum(), prevBlockTimestamp, currentBlockTimestamp)
		self.msgPool.DropMsg(msg)
		return
	}
	...
	if len(txs) > 0 && self.nonSystxs(txs, msgBlkNum) {
		...
		go func() {
			if err := self.poolActor.VerifyBlock(txs, validHeight); err != nil && err != actor.ErrTimeout {
				...
				log.Warnf("server %d verify proposal blk from %d timeout, blk %d, txs %d, %s",
					self.Index, msg.Block.getProposer(), msgBlkNum, len(txs), err)
			}
			for _, tx := range txs {
			...
			if msgBlkNum == self.GetCurrentBlockNo() {
				...
				if proposer, forEmpty, done := self.blockPool.commitDone(msgBlkNum, self.config.C, self.config.N); done {
					...
					if self.isCommitter(msgBlkNum, self.Index) {
						...
						if err := self.makeCommitment(proposal, msgBlkNum, forEmpty); err != nil {
							log.Errorf("server %d consensused %d, committer broadcast commit msg,err: %s", self.Index, msgBlkNum, err)
						}
					}
					...
					log.Warnf("commit block:%d did not reach done, maybe not enough endorse sig", msgBlkNum)
				}
			} else {
				// nothing to do besides adding to msg pool
				log.Warnf("msgBlkNum:%d != server current block num:%d", msgBlkNum, self.GetCurrentBlockNo())
				...
				for {
					...
					if proposer == math.MaxUint32 {
						if err := self.catchConsensus(blkNum); err != nil {
							log.Errorf("server %d fastforward done, catch consensus: %s", self.Index, err)
						}
...
func (self *Server) endorseBlock(proposal *blockProposalMsg, forEmpty bool) error {
	...
	if forEmpty || self.isEndorser(blkNum, self.Index) {
		...
		log.Infof("endorser %d, endorsed block %d, from server %d,and broadcast all peer",
			self.Index, blkNum, proposal.Block.getProposer())
...
func (self *Server) commitBlock(proposal *blockProposalMsg, forEmpty bool) error {
	...
	if forEmpty || self.isCommitter(blkNum, self.Index) {
		...
		log.Infof("committer %d, set block %d committed, from server %d,and broadcast all peer",
			self.Index, blkNum, proposal.Block.getProposer())
...
func (self *Server) makeProposal(blkNum uint32, forEmpty bool) error {
	if blkNum < self.GetCurrentBlockNo() {
		return fmt.Errorf("server %d ignore deprecated blk proposal %d, current %d",
	...
	log.Infof("server %d make proposal for block %d,and broadcast all peer", self.Index, blkNum)
...
func (self *Server) fetchProposal(blkNum uint32, proposer uint32) error {
	...
	log.Infof("fetchProposal blkNum:%d,proposer:%d,and broadcast all peer", blkNum, proposer)
	self.msgSendC <- &SendMsgEvent{
...
func (self *Server) catchConsensus(blkNum uint32) error {
	...
	if len(eMsgs) > C {
		...
		log.Infof("server %d consensus quorum:%d,maxCnt:%d,emptyCnt:%d", self.Index, C, maxCnt, emptyCnt)
	}
